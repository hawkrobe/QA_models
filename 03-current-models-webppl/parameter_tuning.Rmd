...
urlfile = "https://raw.githubusercontent.com/hawkrobe/QA_models/main/03-current-models-webppl/data/case_study_2_parameter_search.csv"
...
model_human_3_overall %>%
  filter(policyAlpha %in% params_3_overall$policyAlpha &
         questionerAlpha %in% params_3_overall$questionerAlpha &
         R1Alpha %in% params_3_overall$R1Alpha &
         relevanceBetaR1 %in% params_3_overall$relevanceBetaR1 &
         costWeight %in% params_3_overall$costWeight) %>%
  select(answerType, prob, human_prop) %>%
  pivot_longer(cols = c(prob, human_prop), names_to = "agent", values_to = "prop") %>%
  ggplot(aes(x = agent, y = prop, fill = answerType)) +
    geom_bar(position="stack", stat="identity") +
